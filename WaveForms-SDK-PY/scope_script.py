...
def open(device_data, sampling_frequency= 8e06 , buffer_size= 400, offset=0, amplitude_range=3):
    """
...
def trigger(device_data, enable = True, source=trigger_source.analog, channel=1, timeout=0, edge_rising=True, level=-0.3):
    """
        set up triggering
...
try:
    ...
    # Converting secs to microseconds
    time_ms = [i * (1e06  / data.sampling_frequency) for i in range(len(recorded_data))]
